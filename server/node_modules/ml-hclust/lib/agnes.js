...
export function agnes(data, options = {}) {
    const { distanceFunction = euclidean, method = 'complete', isDistanceMatrix = false, } = options;
    let distanceData;
    if (!isDistanceMatrix) {
        distanceData = getDistanceMatrix(data, distanceFunction);
    }
    else {
        distanceData = data;
    }
        ...
        for (let i = 1; i < newDistanceMatrix.rows; i++) {
            ...
            for (let j = 0; j < i; j++) {
                if (j === 0) {
                    ...
                    const val = updateFunction(dKI, dKJ, distance, cluster1.size, cluster2.size, prevICluster.size);
...
function previous(newIndex, row, column) {
    return getPreviousIndex(newIndex, Math.min(row, column), Math.max(row, column));
}
...
function getUpdateFunction(method) {
